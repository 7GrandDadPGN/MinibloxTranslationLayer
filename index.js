...
const VERSION = "3.35.39",
	CELL_VOLUME = 16 * 16 * 16;
const DEG2RAD = Math.PI / 180,
	RAD2DEG = 180 / Math.PI;

function translateText(text) {
	for (const [code, color] of Object.entries(COLOR_CODES))
		text = text.replaceAll(code, color);
	return text;
}

function clampToBox(pos, box) {
	return [
		Math.min(Math.max(pos[0], box[0] - 0.3), box[0] + 0.3),
		Math.min(Math.max(pos[1] + 1.62, box[1]), box[1] + 1.8),
		Math.min(Math.max(pos[2], box[2] - 0.3), box[2] + 0.3),
	];
}
...
function updateChunks(x, z, client) {
	...
	for (let checkX = -7; checkX < 7; checkX++) {
		for (let checkZ = -7; checkZ < 7; checkZ++) {
			...
			if (chunks.includes(pos.join()) || queuedChunks.includes(pos.join()))
				continue;
			positions.push(pos);
		}
	}
	...
	positions.sort((a, b) => {
		const aDist = Math.sqrt(
			(a[0] - Math.floor(x / 16)) * (a[0] - Math.floor(x / 16)) +
				(a[1] - Math.floor(z / 16)) * (a[1] - Math.floor(z / 16))
		);
		const bDist = Math.sqrt(
			(b[0] - Math.floor(x / 16)) * (b[0] - Math.floor(x / 16)) +
				(b[1] - Math.floor(z / 16)) * (b[1] - Math.floor(z / 16))
		);
		return bDist - aDist;
	});
	...
	for (const chunk of chunks) {
		if (!currentlyLoaded.includes(chunk)) {
			...
			const cX = Number.parseInt(split[0]),
				cZ = Number.parseInt(split[1]);
		...
		for (let x = 0; x < 16; x++) {
			for (let y = 0; y < 16; y++) {
				for (let z = 0; z < 16; z++) {
					...
					const blockdata =
						BLOCKS[cell.palette[array.get(offset)]] ?? BLOCKS[9];
					for (let skyY = 0; skyY < 256; skyY++) {
						chunk.setSkyLight(new Vec3(x, skyY, z), 15);
					}
					if (cell.palette.length <= 0) continue;
					chunk.setBlockType(
						new Vec3(x, cell.y + y, z),
						typeof blockdata == "number" ? blockdata : blockdata[0]
					);
					chunk.setBlockData(
						new Vec3(x, cell.y + y, z),
						typeof blockdata == "number" ? 0 : blockdata[1]
					);
				}
			}
		}
...
function spawnEntity(packet, client) {
	playerRotations[packet.id] = [
		convertAngle(packet.yaw, 180),
		convertAngle(packet.pitch),
	];
	playerGamemodes[packet.id] = GAMEMODES[packet.gamemode ?? "survival"];
	playerSkins[packet.id] = packet.cosmetics.skin;
	if (
		playerUUIDs[packet.id] == undefined ||
		playerPositions[packet.id] == undefined
	)
		return;
...
function sendActions() {
	const newStates = {
		punching: lStates[0] > Date.now(),
		sprinting: lStates[1],
		sneak: lStates[2],
	};
	if (
		newStates.punching == lastLState.punching &&
		newStates.sprinting == lastLState.sprinting &&
		newStates.sneak == lastLState.sneak
	)
		return;
	ClientSocket.sendPacket(
...
async function connect(client, requeue, gamemode) {
	...
	ClientSocket.on("CPacketBlockUpdate", (packet) => {
		if (
			!chunks.includes(
				[Math.floor(packet.x / 16), Math.floor(packet.z / 16)].join()
			)
		)
			return;
		...
		client.write("block_change", {
			...
			type:
				((typeof blockdata == "number" ? blockdata : blockdata[0]) << 4) |
				(typeof blockdata == "number" ? 0 : blockdata[1]),
		});
	});
	ClientSocket.on("CPacketChunkData", (packet) => {
		const chunk = createChunk(packet),
			chunkInd = [packet.x, packet.z].join();
	...
	ClientSocket.on("CPacketCloseWindow", (packet) =>
		client.write("close_window", { windowId: packet.windowId })
	);
			...
			switch (watched.objectType) {
				case 2:
					...
					if (watched.dataValueId != 7 && watched.dataValueId != 18) {
						wType = 0;
						value =
							watched.dataValueId == 10 ? 127 : convertToByte(watched.intValue);
					}
	...
	ClientSocket.on("CPacketEntityRelPositionAndRotation", (packet) => {
		...
		const yaw =
			packet.yaw != undefined
				? convertAngle(packet.yaw, 180)
				: playerRotations[packet.id][0];
		const pitch =
			packet.pitch != undefined
				? convertAngle(packet.pitch)
				: playerRotations[packet.id][1];
	...
	ClientSocket.on("CPacketPlayerList", (packet) => {
		...
		for (const entity of packet.players) {
			const nameSplit =
				entity.id == clientId ? [client.username] : entity.name.split(" ");
			const realName = nameSplit[nameSplit.length - 1].slice(0, 16);
			const uuid = entity.id == clientId ? client.uuid : entity.uuid;
			const skin =
				playerSkins[entity.id] != undefined &&
				SKINS[playerSkins[entity.id]] != undefined
					? SKINS[playerSkins[entity.id]]
					: SKINS.granddad;
			...
			newData.push({
				...
				gamemode:
					playerGamemodes[entity.id] != undefined
						? playerGamemodes[entity.id]
						: 0,
				ping: entity.ping,
			});
			client.write("scoreboard_team", {
				...
				prefix: (
					...
					translateText(
						`\\${
							(entity.color != "white" ? entity.color : undefined) ??
							(entity.id == clientId ? "white" : "reset")
						}\\`
					)
				).slice(0, 16),
				suffix: "",
	...
	ClientSocket.on("CPacketSetSlot", (packet) => {
		client.write("set_slot", {
			windowId: packet.windowId,
			slot:
				packet.windowId == 0 && SLOTS[packet.slot] != undefined
					? SLOTS[packet.slot]
					: packet.slot,
	...
	ClientSocket.on("CPacketSpawnEntity", (packet) => {
		...
		client.write("spawn_entity", {
			...
			objectData: {
				intField:
					packet.shooterId != null
						? packet.shooterId == clientId
							? mcClientId
							: packet.shooterId
						: 1,
				velocityX: Math.max(Math.min(packet.motion.x * 8000, 32767), -32768),
	...
	ClientSocket.on("CPacketTabComplete", (packet) =>
		client.write("tab_complete", { matches: packet.matches })
	);
		...
		if (packet.id == clientId) {
			...
			lastLHP[1] =
				packet.food != undefined
					...
					: 20;
			lastLHP[2] =
				packet.foodSaturation != undefined
					? packet.foodSaturation
					: lastLHP[2] != undefined
					? lastLHP[2]
					: 20;
				...
				if (playerUUIDs[packet.id] != undefined) {
					client.write("player_info", {
						...
						data: [
							{
								UUID: playerUUIDs[packet.id],
								gamemode: playerGamemodes[packet.id],
							},
						],
					});
				}
...
server.on("playerJoin", async function (client) {
	...
	client.on("chat", (packet) => {
		if (
			packet.message.toLocaleLowerCase().startsWith("/queue") ||
			packet.message.toLocaleLowerCase().startsWith("/play")
		) {
	...
	client.on("held_item_slot", (packet) =>
		ClientSocket.sendPacket(
			new SPacketHeldItemChange({ slot: packet.slotId ?? 0 })
		)
	);
	...
	client.on("use_entity", (packet) => {
		if (
			...
			playerPositions[packet.target] &&
			playerPositions[mcClientId]
		) {
			const newPos = clampToBox(
				playerPositions[mcClientId],
				playerPositions[packet.target]
			);
			ClientSocket.sendPacket(
	...
	client.on("block_dig", (packet) => {
		...
		switch (packet.status) {
			case 2:
				ClientSocket.sendPacket(
					new SPacketBreakBlock({ location: location, start: false })
				);
				break;
			case 0:
				ClientSocket.sendPacket(new SPacketClick({ location: location }));
				ClientSocket.sendPacket(
					new SPacketBreakBlock({ location: location, start: true })
				);
				break;
		}
		ClientSocket.sendPacket(
	...
	client.on("close_window", (packet) =>
		ClientSocket.sendPacket(
			new SPacketCloseWindow({ windowId: packet.windowId })
		)
	);
